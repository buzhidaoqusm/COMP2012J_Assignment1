...
// called from main() to run the simulation.  Summary-Results-FIFO
// is where the summary results are written, and Summary-Processes-FIFO
...
public class Scheduling {
  ...
  private static void Init(String file) {
    ...
    try {   
      ...
      while ((line = in.readLine()) != null) {
        ...
        if (line.startsWith("timeslice")) {
          StringTokenizer st = new StringTokenizer(line);
          st.nextToken();
          timeSlice = Common.s2i(st.nextToken());
        }
        if (line.startsWith("test")) {
          StringTokenizer st = new StringTokenizer(line);
          st.nextToken();
          test = Common.s2i(st.nextToken());
        }
  ...
  public static void main(String[] args) {
    ...
    result = SchedulingAlgorithm.Run(runtime, processVector, result, timeSlice);
    try {
